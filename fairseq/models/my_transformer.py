...
@register_model('transformer_robust')
class TransformerRobustModel(FairseqRobustModel):
    """Robust model for transformer
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--k-only', default=False, action='store_true',
                            help='select the relative mode to map relative position information')
        #add robust model args
        parser.add_argument('--lamda', default=0.5, help='the weight for compute Q of target')
        parser.add_argument('--ratio-pair', default='(0.25, 0.50)', help='the ratio pari for sampling')
        parser.add_argument('--top-n', type=int, default=10, help='top n for select word candidates')

        #add language model args
    ...
    def build_model(cls, args, task):
        ...
        #build lm
        if hasattr(args, 'lmno_tie_adaptive_proj') and args.lmno_tie_adaptive_proj is False:
            # backward compatibility
            args.lmtie_adaptive_proj = True
        ...
        if args.lmcharacter_embeddings:
        	'''
            embed_tokens = CharacterTokenEmbedder(
                task.dictionary, eval(args.lmcharacter_filters),
                args.lmcharacter_embedding_dim, args.lmdecoder_embed_dim,
                args.lmchar_embedder_highway_layers,
            )
            '''
            raise Exception, 'current robust model do not support character embeddings of lm'
        elif args.lmadaptive_input:
        	'''
            embed_tokens = AdaptiveInput(
                len(task.dictionary), task.dictionary.pad(), args.lmdecoder_input_dim,
                args.lmadaptive_input_factor, args.lmdecoder_embed_dim,
                options.eval_str_list(args.lmadaptive_input_cutoff, type=int),
            )
            '''
            raise Exception, 'current robust model do not support adpative input of lm'
        else:
            src_embed_tokens = encoder_embed_tokens
            tgt_embed_tokens = decoder_embed_tokens
        ...
        if args.lmtie_adaptive_weights:
            ...
            assert args.decoder_input_dim == args.decoder_output_dim
			'''
			raise Exception, 'current robust model do not support tie adpative weight of lm'
        ...
        srclm = TransformerDecoder(newlmargs, src_dict, src_embed_tokens, no_encoder_attn=True, final_norm=False)
        tgtlm = TransformerDecoder(newlmargs, tgt_dict, tgt_embed_tokens, no_encoder_attn=True, final_norm=False)
        return TransformerRobustModel(srclm, tgtlm, encoder, decoder)

    def AdvGen(self, inputs, q, d, r):
        """
        q: likelihood function
        d: distribution for word sampling
        r: sampling ratio
        """
        select = inputs.new_empty(input.shape[:1]).uniform_(0, 1)
        select = select.ge(r)
...
@register_model_architecture('transformer_robust', 'transformer_robust')
def base_robust_architecture(args):
	#nmt
	args.encoder_embed_path = getattr(args, 'encoder_embed_path', None)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 2048)
    ...
    args.lmdecoder_output_dim = getattr(args, 'lmdecoder_output_dim', args.decoder_embed_dim)
    args.lmdecoder_input_dim = getattr(args, 'lmdecoder_input_dim', args.decoder_embed_dim)

    # The model training is not stable without this
    args.lmdecoder_normalize_before = True

    args.lmadaptive_input = getattr(args, 'lmadaptive_input', False)
    args.lmadaptive_input_factor = getattr(args, 'lmadaptive_input_factor', 4)
    args.lmadaptive_input_cutoff = getattr(args, 'lmadaptive_input_cutoff', None)
